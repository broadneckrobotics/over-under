...
aivision::colordesc SensorGPT__Green(2, 39, 243, 84, 13, 0.51);
// AI Vision Code Definitions
vex::aivision SensorGPT(PORT20, SensorGPT__Green);
...
void trackObjects() {
  ...
  while (true) {
    SensorGPT.takeSnapshot(SensorGPT__Green);

    // If the Vision Sensor detects a matching colored object...
    if (SensorGPT.largestObject.exists) {
      Brain.Screen.clearLine();
      Brain.Screen.print("here");
      if (SensorGPT.largestObject.centerX < centerFOV + offsetX) {
        Drivetrain.turn(left);
      } else if (SensorGPT.largestObject.centerX > centerFOV - offsetX) {
        Drivetrain.turn(right);
      } else { // else...(it is roughly centered) stop turning
        centered = true;
      }
  ...
  //   Brain.Screen.clearLine();
  //   SensorGPT.takeSnapshot(SensorGPT__Green);
  //   bool objectPresent = SensorGPT.largestObject.exists;
  //   if (!objectPresent) {
  //     Drivetrain.stop(brake);
  //   }
  //   centered = false;

  //   while (!centered && objectPresent) {
  //     SensorGPT.takeSnapshot(SensorGPT__Green);
  //     if (!SensorGPT.largestObject.exists) break;
  //     vex::vision::object obj = SensorGPT.largestObject;
  //     // Turn left if object on left
  //     if (obj.centerY < 158 - 15) {
  //       Brain.Screen.print("left");
  //       Drivetrain.turn(right);
  //     }
  //     // Turn right if object on right
  //     else if (obj.centerY > 158 + 15) {
  ...
  //   if (centered && (DistanceSensor.objectDistance(mm) > 400)) {
  //     // Drivetrain.drive(forward);
  //   }
  //   else {
  //     Drivetrain.stop(brake);
...
void moreSpeed() {
  ...
  leftMotorA.setMaxTorque(20, percent);
  leftMotorB.setMaxTorque(20, percent);
  rightMotorA.setMaxTorque(20, percent);
  rightMotorA.setMaxTorque(20, percent);
}
...
int main() {
  ...
  Controller1.ButtonUp.pressed(moreSpeed);
  Controller1.ButtonDown.pressed(moreTorque);
